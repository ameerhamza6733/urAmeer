...
public class RotatingAnimator implements BarParamsAnimator {
    ...
    public void animate() {
        if (this.isPlaying) {
            ...
            if (currTimestamp - this.startTimestamp > DURATION) {
                this.startTimestamp += DURATION;
            }
            ...
            for (RecognitionBar bar : this.bars) {
                ...
                if (i > 0 && delta > DECELERATE_ROTATION_DURATION) {
    ...
    private float decelerate(long delta, int scale) {
        return (((float) scale) * ACCELERATION_ROTATION_DEGREES) + ((-new AccelerateDecelerateInterpolator().getInterpolation(((float) (delta - DECELERATE_ROTATION_DURATION)) / 1000.0f)) * (((float) scale) * ACCELERATION_ROTATION_DEGREES));
    }
